...
def scale_seq(X_train: np.ndarray, X_test: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    nte = X_test.shape[0]
    scaler = StandardScaler()
    tr = scaler.fit_transform(X_train.reshape(-1, f)).reshape(ntr, w, f)
    te = scaler.transform(X_test.reshape(-1, f)).reshape(nte, w, f)
...
def get_classifiers(seed: int) -> dict[str, Any]:
    return {
